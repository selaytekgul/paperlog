...
export function PaperDetail({ paper, user }: { paper: Paper; user: ChatGPTUser | null }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/papers/${paper.id}/logs`).then(async (response) => await response.json() as { logs?: PaperLog[] }),
